...
### 5.8 `insight-agent` 出力からの受け渡し要件

`insight-agent` の `output_schema_v2` は本ツールの直接入力ではなく、`roadmap-request.schema.json` へ正規化して受け渡す。

最低限の写像要件は次のとおり。

- `problems[*]` または `reasoning_summary.headline` から `problem_statement` を組み立てる
- `insights[*].id` / `statement` を `insights[*].insight_id` / `statement` に写像する
- `risk_notes[*]` と `open_questions[*]` は `constraints` または `known_failures` の材料として扱う
- `evidence_refs[*]` は `ref_id` / `kind` / `summary` を満たす形へ要約する
- `run.run_id` や参照した artifact path は `available_assets` または `notes` に保持し、traceability を失わない

この正規化で失われる情報がある場合は、無理に request schema を拡張せず `notes` または `available_assets` へ退避し、MVP の単一課題・同期実行境界を維持する。
...
- insight-agent の output_schema_v2 から request schema への前処理手順が固定されている
