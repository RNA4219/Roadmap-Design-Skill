...
### 5.8 上流 workflow からの受け渡し要件

insight-agent の output_schema_v2 は本ツールの直接入力ではない。上位 workflow が「このケースをロードマップ化すべきか」を先に判定し、roadmap-request.schema.json を満たす planning-ready な入力だけを本ツールへ渡す。

- 入力対象は 1 件の problem_statement に絞られていること
- insights, constraints, available_assets が最低 1 件ずつ含まれること
- 上流の探索結果から未整理の補足を保持したい場合は、known_failures, evidence_refs, notes, assumptions に退避できること
- 本ツールは workflow の判定ロジックや上流固有 schema には依存しないこと

上流 artifact を参考に request を組み立てることは許容するが、その変換責務は workflow 側に置き、本リポジトリの planner 本体や wrapper に持ち込まない。
...
- `completed` では少なくとも 1 件を `recommended=true` にする
- `partial` または `failed` では 0 件でもよい
- `tradeoffs` を空にしない
...
- 複数方針を比較可能な形で返す
- `tradeoffs` を持つ
- `completed` では少なくとも 1 つに `recommended=true` を付ける
...
- request / response / validation-result / error を JSON Schema で検証可能である
- workflow 判定後の planning-ready request 条件が固定されている
...
### V-03 Validate result

`validate` は `validation-result.schema.json` に準拠した結果を返す。

- 妥当な request: `valid=true`, `errors=[]`
- 不正な request: `valid=false`, `errors` に 1 件以上を格納
- schema version 不一致や必須項目欠落も、`validate` では validation result として返してよい

### V-04 Error envelope

`run` の hard error または `validate` / `run` の内部障害時は、別途 `error.schema.json` に準拠した error envelope を返す。
...
- request / response / validation-result / error が schema validation を通る
- `validate` が invalid request に対して `validation-result.schema.json` 準拠の結果を返せる
...
以下は MVP 契約を維持したまま、実装と運用を一段引き上げるための追加要件である。

### BR-01 `validate` 契約の完全実装

- CLI / HTTP / MCP の `validate` はすべて `validation-result.schema.json` 準拠の JSON を返す
- `schema_version`, `mode`, `valid`, `errors`, `warnings` を省略しない
- `errors[*]` は少なくとも `code`, `field`, `message` を持つ
- hard invalid request でも、`validate` 自体は validation result を返す

### BR-02 schema 配布 I/F の実装

- CLI に `schema` コマンドを持たせる
- HTTP に `GET /v1/roadmaps:schema/{kind}` を持たせる
- MCP に `roadmap.schema` を持たせる
- 取得対象は `request`, `response`, `validation-result`, `error` に限定する

### BR-03 workflow 判定後入力契約の明文化

- insight-agent を含む上流 artifact は参考情報として扱い、本ツールの直接入力契約には含めない
- workflow 側が満たすべき planning-ready 条件を README / requirements / specification / interfaces / runbook に揃えて明記する
- examples/request.from_insight_agent.json は workflow 判定後の request 例として維持する

### BR-04 examples / artifacts / contract test の同期

- README で案内する examples が実在し、contract test と同期している
- `examples/request.from_insight_agent.json` と validation examples を常備する
- artifacts は README の再現コマンドで再生成できる

### BR-05 wrapper 契約の名称一致

- MCP tool 名は `roadmap.plan`, `roadmap.validate`, `roadmap.schema` に揃える
- HTTP endpoint は interfaces.md の契約に揃える
- CLI command 名とヘルプ表示も interfaces.md と一致させる

## 14. 追加ブラッシュアップ受け入れ条件

- `validate` の success / failure が全 wrapper で同じ shape を返す
- schema 配布 I/F から 4 種類の schema を取得できる
- workflow 判定後の request fixture を参考例として残せる
- README と `docs/src/interfaces.md` の導線どおりに examples と commands が実在する
- contract / unit / integration test に追加ブラッシュアップ要件の回帰確認が含まれる
