...
`insight-agent` は課題候補と根拠を発見する上流、Roadmap Design Skill はそれを実装計画へ落とす下流として接続します。

- 上流入力: `insight-agent` の `output_schema_v2`
- 下流入力: `schemas/roadmap-request.schema.json`
- 正規化方針: 生の `output_schema_v2` を直接食べるのではなく、request schema へ写像してから `roadmap` を実行する

最小の写像ルールは次のとおりです。

- `problems[0]` または `reasoning_summary.headline` を `problem_statement` へ落とす
- `insights[*]` を `insights[*]` へ写像する
- `risk_notes[*]` / `open_questions[*]` / `problems[*].next_checks` を `constraints` や `known_failures` の材料に使う
- `evidence_refs[*]` を `evidence_refs[*]` へ要約転記する
- `run`, `routing_plan`, `reasoning_summary` は `available_assets` や `notes` に保持する

接続例は [examples/request.from_insight_agent.json](C:/Users/ryo-n/Codex_dev/Roadmap-Design-Skill/examples/request.from_insight_agent.json) を参照してください。元データの参考例は [insight-agent/artifacts/2512.14982v1.v2.json](C:/Users/ryo-n/Codex_dev/insight-agent/artifacts/2512.14982v1.v2.json) です。
